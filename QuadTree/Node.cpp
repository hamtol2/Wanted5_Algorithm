...
bool Node::Subdivide()
{
	...
	bottomRight 
		= new Node(Bounds(x + halfWidth, y + halfHeight, halfWidth, halfHeight), depth + 1);
...
std::vector<Node::NodeIndex> Node::GetQuads(
	const Bounds& targetBounds) const
{
	// 겹치는 영역을 포함할 배열.
	...
	bool right = targetBounds.GetX() >= centerX
		&& targetBounds.GetXMax() <= bounds.GetXMax();

	// 위쪽 영역에 완전히 포함되는지 확인.
	bool top = targetBounds.GetY() >= y && targetBounds.GetYMax() <= centerY;

	// 아래쪽 영역에 완전히 포함되는지 확인.
	bool bottom = targetBounds.GetY() >= centerY
		&& targetBounds.GetYMax() <= bounds.GetYMax();
